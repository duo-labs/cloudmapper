...
from cloudmapper import BASE_DIR, REPORT_DIR
from .nodes import Account, Region
from .query import query_aws, get_parameter_file
...
def get_account_stats(account, all_resources=False):
    ...
    with open(os.path.join(BASE_DIR, "stats_config.yaml"), "r") as f:
...
def setup_report_dir():
    """Create directory and copy resources"""
    os.makedirs(REPORT_DIR, exist_ok=True)
    # used instead shutil.copytree to allow copying into an existing directory
    copy_tree(os.path.join(BASE_DIR, "web"), REPORT_DIR)
    return REPORT_DIR